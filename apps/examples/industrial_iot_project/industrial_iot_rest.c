...
static void set_config_callback(struct http_client_t *client, struct http_req_message *msg)
{
    ...
    tmp = cJSON_GetObjectItem(config, "pin");
    if (tmp) {
        if (tmp->type != cJSON_String) {
            status = 400;
            strncpy(resp, "{\"error\":true,\"reason\":\"JSON parameter 'pin' should "
                    "be a string\"}", sizeof(resp)-1);
            goto exit;
        }

        if (strlen(tmp->valuestring) > AKC_PIN_LEN) {
            status = 400;
            strncpy(resp, "{\"error\":true,\"reason\":\"JSON parameter 'pin' is too long",
                    sizeof(resp)-1);
            goto exit;
        }

        strncpy(cloud_config.device_pin, tmp->valuestring, AKC_PIN_LEN);
    }
